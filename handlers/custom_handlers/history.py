...
def handle_number_input(message: Message) -> None:
    """
    ...
    if message.text.isdigit():
        queries = database.read_from_db.read_query(message.chat.id)
        number_query = [item[0] for item in queries]
        photo_need = 'yes' if any(int(message.text) == item[0] and item[3] == 'yes' for item in queries) else 'no'
        ...
        if int(message.text) in number_query:
            ...
            if history_dict:
                ...
                for hotel in history_dict.items():
                    send_hotel_info(message, hotel, photo_need)
            else:
...
def send_hotel_info(message: Message, hotel: dict, photo_need: str) -> None:
    """
    Отправляет информацию об отеле пользователю.
    Если пользователь запросил фотографии, отправьте и их.
    """
    medias = []
