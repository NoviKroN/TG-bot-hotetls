...
def need_photo_callback(call: CallbackQuery) -> None:
    """
    Обработчик callback запроса от пользователя после выбора необходимости фотографий.

    Записывает выбор пользователя и переходит к следующему шагу - запросу количества фотографий или даты заезда.
